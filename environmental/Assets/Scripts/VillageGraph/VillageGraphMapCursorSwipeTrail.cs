...
public sealed class VillageGraphMapCursorSwipeTrail : MonoBehaviour
{
    ...
    [Tooltip("Пусто — как у VillageGraphMapCursorSwipeImpulse (GameManager.MapCamera).")]
    [SerializeField]
    ...
    [Tooltip("0 — хвост, 1 — курсор. Доля толщины по длине линии.")]
    [SerializeField]
    ...
    private void Update()
    {
        ...
        var vMin = _impulse.MinSwipeSpeedWorldXY;
        var swipeStrong = geomOk && speed >= vMin;

        if (swipeStrong)
        {
            _lineFade = 1f;
            var vMax = Mathf.Max(_impulse.MaxSwipeSpeedWorldXY, vMin + 1e-4f);
            var t01 = Mathf.InverseLerp(vMin, vMax, Mathf.Clamp(speed, vMin, vMax));
            _lastStrength01 = t01;

            if (_pts.Count == 0 || Vector3.Distance(w1, _pts[_pts.Count - 1]) >= minPointDistance)
            {
                _pts.Add(w1);
                _str.Add(t01);
            ...
            {
                _pts[_pts.Count - 1] = w1;
                _str[_str.Count - 1] = t01;
            }
        }
        else
        {
